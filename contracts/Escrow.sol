...
contract Escrow {
    ...
    event ServicoDepositado(address contratante, uint256 valor, uint256 timestamp);
    ...
    constructor(address _contratante, address _prestador, address _arbitro, address _tokenPagamento, uint256 _valor) {
        contratante = _contratante;
        prestador = _prestador;
        arbitro = _arbitro;
        tokenPagamento = _tokenPagamento;
        valor = _valor;
    ...
    function depositar() public somenteContratante {
        ...
        // O contratante após ter executado o "approve" no token quando de chamar essa função
        // O contrato vai até a carteira do contratante e puxa a quantidade exata de tokens
        bool sucesso = IERC20(tokenPagamento).transferFrom(contratante, address(this), valor);
        require(sucesso, "A transferencia do token falhou!");

        emit ServicoDepositado(contratante, valor, block.timestamp);
    }
    ...
    function pagarPrestador() public somenteContratante {
        ...
        // Transfere o valor exato acordado do token para o prestador
        bool sucesso = IERC20(tokenPagamento).transfer(prestador, valor);
        require(sucesso, "A transferencia falhou!");

        emit PagamentoRealizado(prestador, valor);
    ...
    function reembolsar() public somenteContratante {
        ...
        // Reembolsa o valor exato acordado do token para o contratante
        bool sucesso = IERC20(tokenPagamento).transfer(contratante, valor);
        require(sucesso, "A transferencia falhou!");

        emit ReembolsoRealizado(contratante, valor);
    }
    ...
    function resolverDisputa(bool decisaoDoPrestador) public somenteArbitro {
        ...
        if(decisaoDoPrestador) {
            ...
            bool sucesso = IERC20(tokenPagamento).transfer(prestador, valor);
            require(sucesso, "Falha ao pagar prestador");
            ...
            bool sucesso = IERC20(tokenPagamento).transfer(contratante, valor);
            require(sucesso, "Falha ao reembolsar contratante");
